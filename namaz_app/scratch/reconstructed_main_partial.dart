...
32:   bool _locationSelected = false;
33:   bool _isBlocked = false;
34:   Timer? _blockTimer;
35: 
36:   @override
37:   void initState() {
38:     super.initState();
39:     _initializeApp();
40:     _startBlockCheckTimer();
41:   }
42: 
43:   @override
44:   void dispose() {
45:     _blockTimer?.cancel();
46:     super.dispose();
47:   }
48: 
49:   void _startBlockCheckTimer() {
50:     _blockTimer = Timer.periodic(const Duration(seconds: 3), (timer) async {
51:       _checkBlockStatus();
52:     });
53:   }
54: 
55:   Future<void> _checkBlockStatus() async {
56:     try {
57:       final blocked = await _repository.getGlobalBl
58:     setState(() {
59:       _loading = true;
60:     });
61: 
...
97:   - Orta sütun: "Ücretsiz" (✓ veya — simgeleri)
98:   - Sağ sütun: "Premium 👑" (tüm ✓ işaretleri)
99: - **Fiyat Planları (3 kart)**:
100:   - **Aylık Plan**: Yeşil arka plan, "Aylık ödeme", ₺24,99
101:   - **Önerilen Plan**: Koyu gri/siyah arka plan, "Tek seferlik" etiketi + "Önerilen Planı", "Tek seferlik ödeme", ₺249,99
102:   - **Yıllık Plan**: Turuncu arka plan, "50% İndirim" etiketi, "Yıllık Plan", "Yıllık ödeme", ₺149,99
103: - **Satın Al Butonu**: Tam genişlikte koyu "Satın Al" butonu
104: - **Aboneliği Geri Yükle**: Alt kısımda gri "Aboneliği Geri Yükle" metin linki
105: 
106: ---
107: 
108: ### [Component: Ana Ekran Entegrasyonu]
109: 
110: #### [MODIFY] [main_screen.dart](file:///c:/Users/kral_/Namaz%20vakitleri/namaz_app/lib/screens/main_screen.dart)
111: - Ana sayfadaki "Ezan Bildirimleri" butonuna tıklandığında artık snackbar gösterilmek yerine `NotificationSettingsScreen` ekranına yönlendirilecektir.
112: 
113: #### [MODIFY] [main.dart](file:///c:/Users/kral_/Namaz%20vakitleri/namaz_app/lib/main.dart)
114: - Yeni ekranların import edilmesi
115: 
116: ---
117: 
...
119: 
120: ### Automated Tests
121: - `flutter build apk --debug` komutu çalıştırılarak uygulamanın hatasız derlendiği doğrulanacaktır.
122: 
123: ### Manual Verification
124: - Android Emülatör üzerinde uygulama çalıştırılarak:
125:   1. **Ayarlar Ekranı**: Görsel 1'deki tasarımla birebir uyum kontrol edilecektir.
126:   2. **Bildirim Ayarları**: Tüm radio, checkbox, toggle ve Kaydet butonu test edilecektir.
127:   3. **Premium Ekranı**: Fiyat tabloları, özellik karşılaştırması ve plan seçimi doğrulanacaktır.
128:   4. **Ezan Bildirimleri Butonu**: Ana sayfadaki butonun artık yeni ekrana yönlendirdiği teyit edilecektir.
129: 
130: MISSING
131: MISSING
132: MISSING
133:       bottomNavigationBarTheme: const BottomNavigationBarThemeData(
134:         selectedItemColor: Color(0xFF27A770),
135:         unselectedItemColor: Colors.grey,
136:         backgroundColor: Colors.white,
137:         elevation: 8,
138:       ),
139:       cardTheme: CardThemeData(
...
555: 
556: // Temporary wrapper class to allow MainAppContainer mapping
557: class MainAppContainerWrapper extends StatelessWidget {
558:   final VoidCallback onThemeChanged;
559:   final VoidCallback onLocationReset;
560: 
561:   const MainAppContainerWrapper({
562:     super.key,
563:     required this.onThemeChanged,
564:     required this.onLocationReset,
565:   });
566: 
567:   @override
568:   Widget build(BuildContext context) {
569:     return MainAppContainer(
570:       onThemeChanged: onThemeChanged,
571:       onLocationReset: onLocationReset,
572:     );
573:   }
574: }
575: 
